...
class Blake
  ...
  def initialize(output_size = 512, salt = nil)
    ...
    if @output_words <= 32
      @word_size, @pack_code, @num_rounds, @pi, @rot_off = 4, 'L>', 14, @@pi4, [16, 12, 8, 7]
    else
      @word_size, @pack_code, @num_rounds, @pi, @rot_off = 8, 'Q>', 16, @@pi8, [32, 25, 16, 11]
    end
  ...
  def digest(input, salt = @salt)
    ...
    input << ("\0" * (@block_size - rem)).force_encoding('binary') if rem.positive?
    # set last marker bit
    input[-1] = (input[-1].ord | 0x01).chr if (@output_words % 32).zero?
    # append high-order bytes of input bit length
    input << [total_bits >> 64].pack('Q>').force_encoding('binary') if @word_size == 8
    # append low-order bytes of input bit length
    input << [total_bits & 0xffffffffffffffff].pack('Q>').force_encoding('binary')
